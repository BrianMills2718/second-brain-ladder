...
| 10 | downstream wiring (skill-graph node, positions, registrations) derived, not hand-edited | R8/R14 | ✅ **BUILT** — `graph.ts` now DERIVES one node per module + positions (auto-layout); only the achievement overlay is authored. Adding a module is one edit. + stage↔node parity gate + module-size gate (R14, `docs/PATH_DERIVATION.md`) |

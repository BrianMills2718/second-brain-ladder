...
const CONCEPTS: Concept[] = [
  ...
  { id: "universal-restriction", term: "universal restriction", layer: "logic", band: "practitioner",
    short: "A @c{class} expression: the things *all* of whose @c{property-schema} values are of a type (allValuesFrom) — but it is *vacuously* true for things with no such value (@c{open-world-assumption}).",
    example: "`eats only Plant` — vegans; yet something with no `eats` asserted trivially qualifies.",
  ...
  { id: "defined-class", term: "defined class", layer: "logic", band: "practitioner",
    ...
    example: "Define `Parent ≡ hasChild some Person`; assert `Ada hasChild Bob` and the reasoner infers `Ada a Parent`.",
...
export const PREREQ_WHY: Record<string, string> = {
  ...
  "universal-restriction>open-world-assumption": "its vacuous-truth case is an open-world effect",
